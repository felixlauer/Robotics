...
def jacobian(q):
    """ Used to calculate Cartesian Velocity from Joint Velocity """
    t1, d2, d3, t4 = q
...
def inverse_kinematics_analytical(target_pos, target_phi=0.0):
    """ Analytical IK based on geometric derivation """
    px, py, pz = target_pos
    theta_14 = target_phi 
...
def generate_cubic_heuristic(waypoints, segment_times):
    ...
    # A. Heuristic Velocities (Tangent method)
    ...
    # C. Cubic Coefficients & Evaluation
...
cartesian_list = np.array([
    [2.300, -0.150, 1.050], [2.300, -0.150, 1.070], [2.204, -0.115, 1.070],
    [2.152, -0.026, 1.070], [2.152,  0.026, 1.070],
    [2.204,  0.115, 1.070], [2.300,  0.150, 1.070],
    [2.300,  0.150, 1.050], [2.204,  0.115, 1.050], [2.152,  0.026, 1.050],
    [2.152, -0.026, 1.050], [2.204, -0.115, 1.050], [2.300, -0.150, 1.050]
])
...
# --- 5. PLOTTING (Exact Layout Requested) ---
