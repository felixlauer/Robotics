...
# --- 2. KINEMATICS & JACOBIAN (Verified Correct) ---
def forward_kinematics(q):
    """
    Computes Cartesian Position (x, y, z) from Joint Angles.
    Matches the "New Notation" from Robotics_Coursework (1).pdf
    """
    t1, d2, d3, t4 = q
    phi = t1 - t4
    wx = (d2 + L1) * np.sin(t1)
...
def jacobian(q):
    ...
    Derivatives consistent with the updated FK.
    """
    t1, d2, d3, t4 = q
...
def generate_lspb_custom_times(waypoints, segment_times, acc_limit=1.5):
    ...
    for j in range(4): # Joint Loop
        for k, t in enumerate(t_total):
            ...
            # We use a safety clamp so blending doesn't exceed the segment duration
...
traj_xdd = np.gradient(traj_xd, axis=0) / 0.02 # Numerical Acc
